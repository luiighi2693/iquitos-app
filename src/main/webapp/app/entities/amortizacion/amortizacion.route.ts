...
export class AmortizacionResolve implements Resolve<IAmortizacion> {
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Amortizacion> {
        const id = route.params['id'] ? route.params['id'] : null;
        if (id) {
            return this.service.find(id).pipe(
                filter((response: HttpResponse<Amortizacion>) => response.ok),
                map((amortizacion: HttpResponse<Amortizacion>) => amortizacion.body)
            );
        }
