...
describe('Operation e2e test', () => {
    ...
    it('should create and save Operations', async () => {
        ...
        await operationComponentsPage.clickOnCreateButton();
        await promise.all([
            operationUpdatePage.setInitDateInput('2000-12-31'),
            operationUpdatePage.setGlossInput('gloss'),
            operationUpdatePage.setAmountInput('5'),
            operationUpdatePage.operationTypeSelectLastOption(),
            operationUpdatePage.boxSelectLastOption(),
            operationUpdatePage.paymentTypeSelectLastOption(),
            operationUpdatePage.conceptOperationInputSelectLastOption(),
            operationUpdatePage.conceptOperationOutputSelectLastOption()
        ]);
        expect(await operationUpdatePage.getInitDateInput()).to.eq('2000-12-31');
        expect(await operationUpdatePage.getGlossInput()).to.eq('gloss');
