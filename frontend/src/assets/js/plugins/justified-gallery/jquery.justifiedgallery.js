  ...
  $.fn.justifiedGallery = function(options) {
    ...
    return this.each(function(index, cont) {
      ...
      $(cont)
        ...
        .each(function(index, entry) {
          ...
          images[index]['src'] =
            typeof $(imgEntry).data('safe-src') != 'undefined' ? $(imgEntry).data('safe-src') : $(imgEntry).attr('src');
          ...
          $(img).load(function() {
            ...
            else images[index]['width'] = this.width;
            images[index]['height'] = settings.rowHeight;

            var usedSizeRangeRegExp = new RegExp(
              '(' +
                settings.sizeRangeSuffixes.lt100 +
                '|' +
          ...
          $(img).error(function() {
            $(cont).prepend(
              getErrorHtml('The image can\'t be loaded: "' + images[index]['src'] + '"', 'jg-usedPrefixImageNotFound')
            );
            images[index] = null;
            if (++loaded == images.length) startProcess(cont, images, settings);
          });
    ...
    function buildContRow(row, images, extraW, settings) {
      ...
      for (var j = 0; j < row.length; j++) {
        row[j]['nh'] = Math.ceil(
          images[row[j]['indx']]['height'] *
            ((images[row[j]['indx']]['width'] + extraW) / images[row[j]['indx']]['width'])
        );
    ...
    function processesImages($, cont, images, lastRowWidth, settings) {
      ...
      $(cont)
        ...
        .error(function() {
          $(cont).prepend(
            getErrorHtml('The image can\'t be loaded: "' + $(this).attr('src') + '"', 'jg-resizedImageNotFound')
          );
        })
        .each(function() {
