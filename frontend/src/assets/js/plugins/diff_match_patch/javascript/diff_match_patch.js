(function() {
  ...
  diff_match_patch.prototype.diff_bisect_ = function(a, b, c) {
    for (
      var d = a.length, e = b.length, f = Math.ceil((d + e) / 2), g = f, h = 2 * f, j = Array(h), i = Array(h), k = 0;
      k < h;
      k++
    )
      (j[k] = -1), (i[k] = -1);
  ...
  diff_match_patch.prototype.match_bitap_ = function(a, b, c) {
    ...
    for (var j = 1 << (b.length - 1), h = -1, i, k, q = b.length + a.length, r, t = 0; t < b.length; t++) {
      ...
      for (k[p + 1] = (1 << t) - 1; p >= i; p--) {
        ...
        k[p] =
          0 === t ? ((k[p + 1] << 1) | 1) & w : (((k[p + 1] << 1) | 1) & w) | (((r[p + 1] | r[p]) << 1) | 1) | r[p + 1];
  ...
  diff_match_patch.prototype.patch_addContext_ = function(a, b) {
    if (0 != b.length) {
      for (
        ...
        b.indexOf(c) != b.lastIndexOf(c) && c.length < this.Match_MaxBits - this.Patch_Margin - this.Patch_Margin;
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
      ...
      if (h.length > this.Match_MaxBits) {
        if (
          ...
          -1 != j &&
            ((i = this.match_main(b, h.substring(h.length - this.Match_MaxBits), g + h.length - this.Match_MaxBits)),
            -1 == i || j >= i))
        )
          j = -1;
      ...
      if (-1 == j) (e[f] = !1), (d -= a[f].length2 - a[f].length1);
        ...
        h.length > this.Match_MaxBits && this.diff_levenshtein(g) / h.length > this.Patch_DeleteThreshold)
      )
        e[f] = !1;
      else {
            ...
            1 === g
              ...
              : -1 === g && 1 == h.diffs.length && 0 == h.diffs[0][0] && i.length > 2 * b
                ...
                : ((i = i.substring(0, b - h.length1 - this.Patch_Margin)),
                  ...
                  h.diffs.push([g, i]),
                  i == d.diffs[0][1] ? d.diffs.shift() : (d.diffs[0][1] = d.diffs[0][1].substring(i.length)));
          }
