(function(root, factory) {
    ...
    update: function(e) {
      ...
      this.graph.series.active().forEach(function(series) {
        ...
        var distance = Math.sqrt(
          Math.pow(Math.abs(graph.x(value.x) - eventX), 2) + Math.pow(Math.abs(graph.y(value.y + value.y0) - eventY), 2)
        );
  ...
  Rickshaw.Graph.RangeSlider.Preview = Rickshaw.Class.create({
    ...
    _renderDimming: function() {
      ...
      path += ' z ';
      path +=
        ' M ' + Math.max(this.currentFrame[0], this.config.frameHandleThickness) + ' ' + this.config.frameTopThickness;
      path +=
        ' H ' +
        Math.min(
  ...
  Rickshaw.Graph.Renderer.Bar = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {
    name: 'bar',
    defaults: function($super) {
      var defaults = Rickshaw.extend($super(), { gapSize: 0.05, unstack: false });
