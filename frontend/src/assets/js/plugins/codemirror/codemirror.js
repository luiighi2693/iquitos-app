...
(function(mod) {
  ...
  function CodeMirror(place, options) {
    ...
    if ((options.autofocus && !mobile) || activeElt() == display.input) setTimeout(bind(onFocus, this), 20);
    else onBlur(this);
    ...
    for (var first = true; ; first = false) {
      if (first && cm.options.lineWrapping && update.oldScrollerWidth != cm.display.scroller.clientWidth) {
        ...
        if (viewport && viewport.top != null)
          viewport = {
            top: Math.min(
              cm.doc.height + paddingVert(cm.display) - scrollerCutOff - cm.display.scroller.clientHeight,
              viewport.top
            )
          };
  ...
  function insertLineWidgets(lineView, dims) {
    ...
    if (lineView.rest)
      for (var i = 0; i < lineView.rest.length; i++) insertLineWidgetsFor(lineView.rest[i], lineView, dims, false);
  }
    ...
    if (node.nodeType == 3) {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        if (ie && ie_version < 9 && start == 0 && end == mEnd - mStart) {
          ...
          if (rects.length) rect = rects[bias == 'right' ? rects.length - 1 : 0];
          else rect = nullRect;
        } else {
  ...
  function endOperation_R2(op) {
    ...
    if (display.maxLineChanged && !cm.options.lineWrapping) {
      ...
      op.maxScrollLeft = Math.max(
        0,
        display.sizer.offsetLeft + op.adjustWidthTo + scrollerCutOff - display.scroller.clientWidth
      );
    }
  ...
  function endOperation_finish(op) {
    ...
    if (op.updatedDisplay && webkit) {
      if (cm.options.lineWrapping) checkForWebkitWidthBug(cm, op.barMeasure); // (Issue #2420)
      if (
  ...
  function readInput(cm) {
    ...
    if (text.charCodeAt(0) == 0x200b && doc.sel == cm.display.selForContextMenu && !prevInput) prevInput = '\u200b';
  ...
  function resetInput(cm, typing) {
    ...
    if (cm.somethingSelected()) {
      ...
      minimal =
        hasCopyEvent && (range.to().line - range.from().line > 100 || (selected = cm.getSelection()).length > 1000);
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (e.altKey) {
      ...
      if (cm.display.shift || doc.extend) ourRange = extendRange(doc, ourRange, word.anchor, word.head);
      else ourRange = word;
    } else if (type == 'triple') {
      var line = new Range(Pos(start.line, 0), clipPos(doc, Pos(start.line + 1, 0)));
      if (cm.display.shift || doc.extend) ourRange = extendRange(doc, ourRange, line.anchor, line.head);
      else ourRange = line;
    } else {
    ...
    function extendTo(pos) {
      ...
      if (type == 'rect') {
        ...
        if (type != 'single') {
          ...
          else var range = new Range(Pos(pos.line, 0), clipPos(doc, Pos(pos.line + 1, 0)));
  ...
  CodeMirror.prototype = {
    ...
    heightAtLine: function(line, mode) {
      ...
      return (
        intoCoordSystem(this, lineObj, { top: 0, left: 0 }, mode || 'page').top +
  ...
  function conflictingCollapsedRange(doc, lineNo, from, to, marker) {
    ...
    if (sps)
      for (var i = 0; i < sps.length; ++i) {
        ...
        if (
          (fromCmp <= 0 && (cmp(found.to, from) > 0 || (sp.marker.inclusiveRight && marker.inclusiveLeft))) ||
