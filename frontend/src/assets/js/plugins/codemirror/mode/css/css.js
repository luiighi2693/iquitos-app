...
(function(mod) {
  ...
  CodeMirror.defineMode('css', function(config, parserConfig) {
    ...
    states.media = function(type, stream, state) {
      ...
      if (type == 'word') {
        var word = stream.current().toLowerCase();
        if (word == 'only' || word == 'not' || word == 'and') override = 'keyword';
    ...
    states.font_face = function(type, stream, state) {
      if (type == '}') return popContext(state);
      if (type == 'word') {
        if (!fontProperties.hasOwnProperty(stream.current().toLowerCase())) override = 'error';
    ...
    return {
      ...
      indent: function(state, textAfter) {
        ...
        if (
          cx.prev &&
          ((ch == '}' &&
            (cx.type == 'block' || cx.type == 'top' || cx.type == 'interpolation' || cx.type == 'font_face')) ||
