...
(function(mod) {
  ...
  CodeMirror.defineMode('python', function(conf, parserConf) {
    ...
    var doubleOperators =
      parserConf.doubleOperators || new RegExp('^((==)|(!=)|(<=)|(>=)|(<>)|(<<)|(>>)|(//)|(\\*\\*))');
    var doubleDelimiters =
      parserConf.doubleDelimiters || new RegExp('^((\\+=)|(\\-=)|(\\*=)|(%=)|(/=)|(&=)|(\\|=)|(\\^=))');
    ...
    function tokenBase(stream, state) {
      ...
      if (stream.sol() && top(state).type == 'py') {
        ...
        if (stream.eatSpace()) {
          ...
          if (lineOffset > scopeOffset) pushScope(stream, state, 'py');
          else if (lineOffset < scopeOffset && dedent(stream, state)) state.errorToken = true;
          return null;
        } else {
    ...
    function tokenStringFactory(delimiter) {
      while ('rub'.indexOf(delimiter.charAt(0).toLowerCase()) >= 0) delimiter = delimiter.substr(1);
    ...
    function pushScope(stream, state, type) {
      ...
      if (type != 'py' && !stream.match(/^(\s|#.*)*$/, false)) align = stream.column() + 1;
    ...
    function tokenLexer(stream, state) {
      ...
      if (current == ':' && !state.lambda && top(state).type == 'py') pushScope(stream, state, 'py');

      var delimiter_index = current.length == 1 ? '[({'.indexOf(current) : -1;
      if (delimiter_index != -1) pushScope(stream, state, '])}'.slice(delimiter_index, delimiter_index + 1));
    ...
    var external = {
      ...
      indent: function(state, textAfter) {
        ...
        else if (closing && state.scopes.length > 1) return state.scopes[state.scopes.length - 2].offset;
