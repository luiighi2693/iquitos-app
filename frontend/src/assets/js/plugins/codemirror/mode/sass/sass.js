  ...
  CodeMirror.defineMode('sass', function(config) {
    ...
    function tokenBase(stream, state) {
      ...
      if (ch === '#') {
        ...
        // Hex numbers
        if (stream.match(/[0-9a-fA-F]{6}|[0-9a-fA-F]{3}/)) return 'number';
      ...
      if (stream.eatWhile(/[\w-&]/)) {
        ...
        if (stream.peek() === ':' && !stream.match(pseudoElementsRegexp, false)) return 'property';
        else return 'atom';
      }
    ...
    function tokenLexer(stream, state) {
      ...
      if (current === '@return') dedent(state);
