...
(function(mod) {
  ...
  function stringWithEscapes_(stream, state, closing) {
    ...
    if (stream.match(/^\$[a-zA-Z_][a-zA-Z0-9_]*/)) {
      ...
      if (stream.match('[', false)) {
        ...
        state.tokenize = matchSequence(
          [
            [['[', null]],
            [[/\d[\w\.]*/, 'number'], [/\$[a-zA-Z_][a-zA-Z0-9_]*/, 'variable-2'], [/[\w\$]+/, 'variable']],
            [[']', null]]
          ],
          closing
        );
      ...
      if (stream.match(/\-\>\w/, false)) {
        ...
        state.tokenize = matchSequence([[['->', null]], [[/[\w]+/, 'variable']]], closing);
      }
      return 'variable-2';
    }
    ...
    while (
      !stream.eol() &&
      (escaped || (!stream.match('{$', false) && !stream.match(/^(\$[a-zA-Z_][a-zA-Z0-9_]*|\$\{)/, false)))
    ) {
