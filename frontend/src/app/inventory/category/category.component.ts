...
export class CategoryComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(CategoryComponent) table: CategoryComponent;
  constructor(private service: CategoriaService, public dialog: MatDialog, public fullService: FullService) {
    this.itemsPerPage = 500;
    this.page = 0;
    this.predicate = 'id';
    this.reverse = true;
    this.fullService.changeMenu(menu);
    this.fullService.changeMenuSelected('CATEGORIAS');
  }
  ...
  loadAll() {
    if (this.currentSearch) {
      this.service
        ...
        .subscribe(
          (res: HttpResponse<ICategoria[]>) => this.paginate(res.body, res.headers),
  ...
  private subscribeToSaveResponse(result: Observable<HttpResponse<ICategoria>>) {
    result.subscribe((res: HttpResponse<ICategoria>) =>
  ...
  private paginate(data: ICategoria[], headers: HttpHeaders) {
